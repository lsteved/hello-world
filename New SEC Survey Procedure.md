...
In order to have the responses only become available at a certain date (i.e. after the grades submission deadline each term) I changed the permissions on the Survey Feedback module in the SEC course so that the results would not be available until I updated the permissions at the start of the “release period”. (I also removed the ability of Professors to Backup activities for the Survey Feedback Activity/module, though they can still back it up as part of a full course backup. And, I Locked “Include permission overrides” in the General restore defaults and in the General import defaults Moodle Admin settings.) I also added a tag to the Survey Feedback module to make it easier to find the courses into which it had been imported each term (either by myself, Tracey, Denise or Amanda). 

## Steps
- Step 1:
Therefore... UPON REQUEST, (after ensuring that the [Student Experience of Courses module in the SEC course](https://moodle.mta.ca/mod/feedback/view.php?id=400586) has the correct tag for the current term; see end of final step below for more details) import the Student Experience of Courses module into the required course(s) and send the following message to the instructor (after updating the dates to the relevant term): 
...
Also, here is a resource related to the survey [Student Experience of Courses Survey NEW.pdf](https://moodle.mta.ca/course/format/tiles/mod_view.php?cmid=31908)   
...
- Step 3:
THEN... on May 31st (or applicable date for that term), go back in and (according to the policy governing the survey) delete the survey data, i.e. delete the module from each course.
...
## Potential issues/pitfalls/notes
- Note 1:
Dates in the above steps will need to be adjusted during each survey cycle based on the following future release period dates (confirmed by Richard Isnor on November 2, 2025):
Winter Term Release Periods, the 2 weeks leading into May 31st of that calendar year; Spring/Summer Term Release Periods, the 2 weeks leading into September 30th of that calendar year; Fall Term Release Periods, the 2 weeks leading into January 31st of the following calendar year.
...
- Note 3:
IMPORTANT UPDATE... Another update, regarding survey anonymity, was implemented on December 4, 2025. The moodle/course:manageactivities capability was removed for all instructors and TAs under the SEC Feedback activity module Permissions, in order to prevent possible changes to survey anonymity or completion tracking settings. This also removed the Settings menu entirely, as well as certain options under the 3 dot menu for the activity on the main course page (although these permission restrictions can be bypassed, if instructors Duplicate the Feedback activity). Therefore certain survey customizations will now need to be carried out by the Educational Technology Consultant, upon request. Instructors can still hide/unhide the surveys, as desired. (The survey resource sent out to instructors has been updated to reflect this change)


## Relevant Links
[Student Experience of Courses module in the SEC course](https://moodle.mta.ca/mod/feedback/view.php?id=400586)

[Student Experience of Courses Survey NEW.pdf](https://moodle.mta.ca/course/format/tiles/mod_view.php?cmid=31908) 
